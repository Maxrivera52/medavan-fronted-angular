...
import { DoctorsFakeData } from '@fake-db/doctors.data';
import { PatientsFakeData } from './patients.data';
import { SpecialtysFakeData } from './specialtys.data';
import { OriginsFakeData } from './origins.data';
import { PreferencesFakeData } from './preferences.data';
import { SuppliersFakeData } from './suppliers.data';
import { OperatorsFakeData } from './operators.data';
import { EquiposFakeData } from './equipos.data';
import { CirugiasFakeData } from './cirugias.data';
import { DiagnosticosFakeData } from './diagnosticos.data';
import { PaymentFakeData } from './payment.data';
import { AnesthesiaFakeData } from './anesthesia.data';
import { TagsFakeData } from './tag.data';
...
export class FakeDbService implements InMemoryDbService {
  createDb(): any {
    return {
      ...
      // Doctors
      'doctors-data': DoctorsFakeData.doctors,

      // Patients
      'patients-data': PatientsFakeData.patients,

      // Equipos
      'equipos-data': EquiposFakeData.equipos,

      // Suppliers
      'suppliers-data': SuppliersFakeData.suppliers,

      // Operators
      'operators-data': OperatorsFakeData.operators,

      // Patients
      'specialtys-data': SpecialtysFakeData.specialys,
  
      // Patients
      'origins-data': OriginsFakeData.origins,

      // Patients
      'preferences-data': PreferencesFakeData.preferences,

      // Invoice
      'invoice-data': InvoiceFakeData.invoices,

      // Diagnosticos
      'diagnosticos-data': DiagnosticosFakeData.diagnosticos,

      // Cirugias
      'cirugias-data': CirugiasFakeData.cirugias,
      // Cirugias
      'payment-data': PaymentFakeData.payments,
      // Cirugias
      'anesthesia-data': AnesthesiaFakeData.anesthesias,
      // Cirugias
      'tags-data': TagsFakeData.tags,
