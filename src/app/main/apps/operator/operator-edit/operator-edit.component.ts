...
import { locale as english }    from 'app/main/apps/operator/operator-edit/i18n/en';
...
export class OperatorEditComponent implements OnInit, OnDestroy {
   ...
   *
   * @param {Router} router
   * @param {OperatorEditService} _operatorEditService
   */
  constructor(private router: Router, private _operatorEditService: OperatorEditService, private _coreTranslationService: CoreTranslationService) {
   ...
   */
  ngOnInit(): void {
    this._operatorEditService.onOperatorEditChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
      this.rows = response;
      this.rows.map(row => {
