...
import { locale as english }    from 'app/main/apps/material/materials-edit/i18n/en';
...
export class MaterialEditComponent implements OnInit, OnDestroy {
   ...
   *
   * @param {Router} router
   * @param {MaterialEditService} _materialEditService
   */
  constructor(private router: Router, private _materialEditService: MaterialEditService, private _coreTranslationService: CoreTranslationService) {
   ...
   */
  ngOnInit(): void {
    this._materialEditService.onMaterialEditChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
      this.rows = response;
      this.rows.map(row => {
