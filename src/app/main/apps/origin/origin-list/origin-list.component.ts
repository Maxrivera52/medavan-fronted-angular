...
import { OriginListService } from "app/main/apps/origin/origin-list/origin-list.service";
...
export class OriginListComponent implements OnInit {
   ...
   * @param {OriginListService} _originListService
...
/*     this._coreConfigService.config
      ...
      .subscribe((config) => {
        ...
        if (config.layout.animation === "zoomIn") {
          setTimeout(() => {
            this._originListService.onOriginListChanged
              .pipe(takeUntil(this._unsubscribeAll))
              .subscribe((response) => {
                this.rows = response;
    ...
    this._originListService.changeList$
      .pipe(
        switchMap(() => this._originListService.getOrigins()),
  ...
  selectOrigin(origin: IOrigin): void {
    this._originListService.originSelected$.next(origin);
    this._coreSidebarService
      .getSidebarRegistry("new-origin-sidebar")
      .toggleOpen();
  }

  deletOrigin(id: number) {
    this._originListService
      .deleteOrigin(id)
      .pipe(takeUntil(this._unsubscribeAll))
      .subscribe((response) => {
        this._originListService.changeList$.next();
