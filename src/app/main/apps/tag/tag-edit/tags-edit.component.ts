...
import { locale as english }    from 'app/main/apps/tag/tag-edit/i18n/en';
...
export class TagEditComponent implements OnInit, OnDestroy {
   ...
   *
   * @param {Router} router
   * @param {TagEditService} _tagEditService
   */
  constructor(private router: Router, private _tagEditService: TagEditService, private _coreTranslationService: CoreTranslationService) {
   ...
   */
  ngOnInit(): void {
    this._tagEditService.onTagEditChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
      this.rows = response;
      this.rows.map(row => {
