...
import { locale as english }    from 'app/main/apps/sede/sede-edit/i18n/en';
...
export class SedeEditComponent implements OnInit, OnDestroy {
   ...
   *
   * @param {Router} router
   * @param {SedeEditService} _sedeEditService
   */
  constructor(private router: Router, private _sedeEditService: SedeEditService, private _coreTranslationService: CoreTranslationService) {
   ...
   */
  ngOnInit(): void {
    this._sedeEditService.onSedeEditChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
      this.rows = response;
      this.rows.map(row => {
