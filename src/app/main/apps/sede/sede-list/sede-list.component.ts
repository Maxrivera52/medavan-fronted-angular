...
import { SedeListService } from "app/main/apps/sede/sede-list/sede-list.service";
...
export class SedeListComponent implements OnInit {
   ...
   * @param {SedeListService} _sedeListService
  ...
  ngOnInit(): void {
    ...
    this._sedeListService.changeList$
      .pipe(
        switchMap(() => this._sedeListService.getSedes()),
  ...
  selectSede(sede: ISede): void {
    this._sedeListService.sedeSelected$.next(sede);
    this._coreSidebarService
      .getSidebarRegistry("new-sede-sidebar")
      .toggleOpen();
  }

  deletSede(id: number) {
    this._sedeListService
      .deleteSede(id)
