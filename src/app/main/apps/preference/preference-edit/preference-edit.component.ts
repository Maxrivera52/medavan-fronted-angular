...
import { locale as english }    from 'app/main/apps/preference/preference-edit/i18n/en';
...
export class PreferenceEditComponent implements OnInit, OnDestroy {
   ...
   *
   * @param {Router} router
   * @param {PreferenceEditService} _preferenceEditService
   */
  constructor(private router: Router, private _preferenceEditService: PreferenceEditService, private _coreTranslationService: CoreTranslationService) {
   ...
   */
  ngOnInit(): void {
    this._preferenceEditService.onPreferenceEditChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
      this.rows = response;
      this.rows.map(row => {
